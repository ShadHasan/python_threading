...
class pool(threading.Thread):
    ...
    # pool without synchronized method

    def __init__(self, max_thread):
        self.max_thread = max_thread
        self.end_flag = True
        for i in range(max_thread):
            self.sample[i] = sample(i,)
        super(pool, self).__init__()

    def action(self, do, data=None):
        allocated = 0
        while allocated == 0:
            for thread_id in self.sample.keys():
                if not self.sample[thread_id].mutex:
                    alloc_pool = thread_id
                    allocated = 1
                    break 
        self.sample[alloc_pool] = sample(alloc_pool,mutex=True,)
        self.sample[alloc_pool].mode = do
        self.sample[alloc_pool].data = data
        self.sample[alloc_pool].start()
...
class sample(threading.Thread):
    ...
    def __init__(self, thread_number, mutex=False):
...
class sync_pool(threading.Thread):
...
class sync_sample(threading.Thread):
